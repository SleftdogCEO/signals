...
export function EnhancedChatWidget() {
  ...
  return (
    <ExpandableChat
      ...
      open={isOpen}
      onOpenChange={setIsOpen} // ✅ Changed from onToggle to onOpenChange
      className="transition-all duration-300"
    >
      <div className="flex flex-col h-full overflow-hidden bg-gradient-to-b from-gray-900 via-gray-900 to-gray-950">        
        <ExpandableChatHeader className="flex items-center justify-between bg-gray-900/95 backdrop-blur-sm px-4 py-3 border-b border-gray-800/50">
          <div className="flex flex-col">
            <h1 className="text-base sm:text-lg font-semibold text-white flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-yellow-500" />
              Sleft Assistant
            </h1>
            <p className="text-xs text-gray-400 hidden sm:block">AI-powered business insights</p>
          </div>
          {/* ✅ Add close button that works on all devices */}
          <Button
            variant="ghost"
            size="icon"
            onClick={() => setIsOpen(false)}
            className="h-8 w-8 text-gray-400 hover:text-white hover:bg-gray-800/50 shrink-0"
          >
            <X className="h-4 w-4" />
          </Button>
        </ExpandableChatHeader>

        {/* ✅ Rest of your component - remove the mobile overlay since it's handled in ExpandableChat */}
        <ExpandableChatBody className="flex-1 flex flex-col overflow-hidden">
          ...
          >
            <TabsList className="w-full sticky top-0 z-10 px-3 sm:px-4 pt-4 pb-2 bg-gray-900/95 backdrop-blur-sm">
              <div className="flex gap-1 p-1 bg-gray-800/50 rounded-lg w-full">
                <TabsTrigger
                  value="ai"
                  className={cn(
                    "flex-1 py-2 px-2 rounded-md transition-all text-xs sm:text-sm font-medium",
                ...
                >
                  ...
                  <span className="hidden xs:inline">Video </span>Avatar
                  {isConnecting && (
                ...
                <div className="h-full">
                  <ChatMessageList className="h-full">
                    ...
                    {isLoading && activeTab === "avatar" && (
                      <ChatBubble variant="received">
                        <ChatBubbleAvatar className="h-6 w-6 sm:h-8 sm:w-8 shrink-0" src="/avatars/ai-avatar.png" fallback="A" />
                        <ChatBubbleMessage isLoading />
                      </ChatBubble>
                    )}
                  </ChatMessageList>
        ...
        <ExpandableChatFooter className="bg-gray-900/95 backdrop-blur-sm border-t border-gray-800/50">
          <form
