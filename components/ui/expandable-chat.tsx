  ...
  return (
    <div className={cn(`fixed ${chatConfig.positions[position]} z-50`, className)} {...props}>
      <div
        ref={chatRef}
        className={cn(
          "flex flex-col bg-gradient-to-b from-gray-900 to-gray-950 border border-gray-800/50 sm:rounded-2xl shadow-2xl overflow-hidden transition-all duration-250 ease-out sm:absolute sm:w-[90vw] sm:h-[80vh] fixed inset-0 w-full h-full sm:inset-auto backdrop-blur-xl backdrop-saturate-150",
...
const ExpandableChatHeader: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => (
  <div
    className={cn(
      "flex items-center justify-between p-4 border-b border-gray-800/50 bg-gradient-to-r from-gray-900 via-yellow-500/10 to-gray-900",
      className,
    )}
    {...props}
  />
...
const ExpandableChatFooter: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => (
  <div className={cn("border-t border-gray-800/50 p-4 bg-gray-900/95", className)} {...props} />
)
...
const ExpandableChatToggle: React.FC<ExpandableChatToggleProps> = ({
  ...
  <Button
    ...
    onClick={toggleChat}
    className={cn(
      "w-14 h-14 rounded-full shadow-lg flex items-center justify-center hover:shadow-xl hover:shadow-yellow-500/20 transition-all duration-300 bg-yellow-500 hover:bg-yellow-600 text-black",
      className,
    )}
    {...props}
  >
